...
# Η γηπεδούχος ομάδα σκοράρει κατά μέσο όρο ~20-25% παραπάνω απ' ό,τι εκτός έδρας.
# Το μοντέλο δεν το λάβαινε καθόλου υπόψη -- προστέθηκε ως διορθωτικό στο compute_expected_goals.
HOME_ADVANTAGE_FACTOR = 1.12    # lam_home *= αυτό, lam_away /= αυτό

# Overdispersion correction: το ποδόσφαιρο έχει περισσότερη variance από το θεωρητικό Poisson.
# Υψηλότερη τιμή = πιο "ακραία" σκορ είναι πιθανά (0-0 και 3-2 συχνότερα, 1-1 σπανιότερα).
# 1.0 = καμία διόρθωση. 1.05-1.15 = τυπικό εύρος για ποδόσφαιρο.
OVERDISPERSION_CORRECTION = 1.10

# Στάθμιση πρόσφατων αγώνων στο team_form_from_fixtures: κάθε προηγούμενο (παλαιότερο)
# ματς βαραίνει FORM_DECAY_FACTOR φορές λιγότερο από το αμέσως πιο πρόσφατο.
# 1.0 = καμία στάθμιση (παλιά συμπεριφορά). Παραδοχή: recent_fixtures έρχεται πιο πρόσφατο πρώτο.
FORM_DECAY_FACTOR = 0.90
...
MIN_MODEL_PROBABILITY = 0.80   # ανέβηκε από 65% -- ακόμα πιο αυστηρό φίλτρο
...
COMBO_BETS_MIN_LEG_PROB = 0.80   # ευθυγραμμισμένο με το νέο γενικό όριο (πριν ήταν 65%)
COMBO_BETS_MIN_LEGS = 2
COMBO_BETS_MAX_LEGS = 2
COMBO_BETS_CORRELATION_PENALTY = 0.90
COMBO_BETS_MIN_COMBINED_PROB = 0.30

# Ζεύγη "market family" που είναι στατιστικά ΙΣΧΥΡΑ συσχετισμένα (ουσιαστικά το ίδιο
# σήμα xG, π.χ. Over Goals + BTTS Yes ανεβαίνουν/κατεβαίνουν μαζί) -- εντοπίστηκε από
# πολύ χαμηλό πραγματικό ποσοστό επιτυχίας Combo Bets (33%, 0%, 17% σε 3 δείγματα).
# Για τέτοια ζεύγη εφαρμόζουμε πολύ αυστηρότερο penalty αντί για το κανονικό.
COMBO_BETS_HIGH_CORRELATION_PAIRS = {
    frozenset({"Goals O/U", "BTTS"}),
    frozenset({"Goals O/U", "Team Goals"}),
    frozenset({"BTTS", "Team Goals"}),
    frozenset({"1X2", "Double Chance"}),
    frozenset({"1X2", "Clean Sheet"}),
    frozenset({"BTTS", "Clean Sheet"}),
}
COMBO_BETS_HIGH_CORRELATION_PENALTY = 0.55   # πολύ πιο αυστηρό από το κανονικό 0.90
